document.addEventListener('DOMContentLoaded', function() {
  ...
  // Tous les états ont des labels qui se RESSEMBLENT volontairement
  var states = [
    ...
    { label: 'Certifié', type: 'good' },    // piège : ressemble à conforme
    { label: 'Contrefait', type: 'bad' },    // piège : ressemble à certifié
    { label: 'Contrôlé', type: 'good' },     // piège : ressemble à contrefait
  ...
  // Couleurs liées au type : VERT = bon, BLEU ou ROUGE = mauvais
  function getColorClass(type) {
    if (type === 'good') return 'etat-vert';
    // Mauvais = aléatoirement bleu ou rouge
  ...
  // VITESSE AGRESSIVE : v1=900ms, v2=650ms, v3=450ms
  function getSpawnDelay() {
    var base = 900 - (currentWave * 200);
    return Math.max(350, base + Math.random() * 200);
  }

  // Temps d'affichage court : v1=1.5s, v2=1.1s, v3=0.85s
  ...
  function showResult(success, finalScore) {
    ...
    if (resultScore) resultScore.textContent = finalScore + ' pts — ' + errors + ' erreurs';
